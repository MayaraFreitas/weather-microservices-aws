...
app.MapGet("/weather-report/{zip}", async (string zip, [FromQuery] int? days, IWeatherReportAggregator weatherReportAgg) =>
{
    if (days == null || days < 0 || days > 30)
        return Results.BadRequest("Please provide a 'days' query parameter between 1 and 30");

    var report = await weatherReportAgg.BuildReport(zip, days.Value);
